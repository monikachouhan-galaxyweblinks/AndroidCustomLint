        ...
        override fun visitClass(node: UClass) {
            ...
            if (methodCounts > STANDARD_METHOD_COUNT) {
                context.report(ISSUE_METHOD_COUNT_EXCEEDED, node,
                        ...
                        "Class contains ${node.psi.name}  ${meth[0].name} ${meth[1].name} ${meth[2].name} ${meth[3].name} $methodCounts  methods, Only $STANDARD_METHOD_COUNT methods are allowed in a class.")
